...
app.get("/api/settings", async (c) => {
  ...
  return c.json(settings || { target_budget: 25e4, currency: "EUR" });
});
app.put("/api/settings", async (c) => {
  ...
  if (existing) {
    ...
    `).bind(userId, target_budget || 25e4, currency || "EUR").run();
  }
...
app.get("/api/expenses", async (c) => {
  ...
  const list = expenses.results.map((row) => ({
    id: row.id,
    user_id: row.user_id,
...
app.get("/api/expenses/total", async (c) => {
  ...
  return c.json({ total: Number(result.total) });
});
app.post("/api/expenses", async (c) => {
  ...
  const now = (/* @__PURE__ */ new Date()).toISOString().replace("Z", "UTC");
  ...
  if (is_regular) {
    ...
    if (existingRegular) {
      return c.json({ error: "Regular expense for this month already exists", id: existingRegular.id }, 409);
    }
  ...
  for (const expense of expenses) {
    ...
    if (existing) {
      ...
      `).bind(
        ...
        expense.purpose,
        expense.date,
        expense.is_regular ? 1 : 0,
        expense.regular_monthly_id || null,
        now,
        expense.id
      ).run();
    } else {
...
app.post("/api/sync/pull", async (c) => {
  ...
  `;
  const params = [userId, lastSyncTimestamp || "1970-01-01", lastSyncTimestamp || "1970-01-01"];
  const expenses = await c.env.DB.prepare(query).bind(...params).all();
  const list = expenses.map((row) => ({
    id: row.id,
    user_id: row.user_id,
    amount: Number(row.amount),
    currency: row.currency,
...
// .wrangler/tmp/bundle-jS1IXc/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-jS1IXc/middleware-loader.entry.ts
